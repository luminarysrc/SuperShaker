...
export default function SuperShakerPanel({ onGcodeGenerated, onNestingDone, onNestingUpdated, settingsVersion, doorsVersion }) {
  ...
  if (error && !settings) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-4" style={{ color: "var(--ss-text-muted)" }}>
        ...
        <button onClick={() => window.location.reload()} className="cnc-btn-primary mt-2">Retry Connection</button>
      </div>
    );
  }
