...
def parse_gcode_with_tools(file_path):
    """
    Парсит G-code, возвращает словарь:
    { 'T6': {'rapid': (x,y,z), 'cut': (x,y,z)}, 'T5': {...}, ... }
    и порядок инструментов.
    """
    x, y, z = 0.0, 0.0, 0.0
    mode = 0  # 0 - G0, 1 - G1/G2/G3
    current_tool = "T0"

    tool_data = {}  # 'T6' -> {'rapid': (x[], y[], z[]), 'cut': (...)}
    tool_order = []
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Поиск смены инструмента: T6, T5 и т.п.
            ...
            # Поиск координат X, Y, Z в строке
            ...
            if new_x != x or new_y != y or new_z != z:
                rapid_xyz = tool_data[current_tool]['rapid']
                cut_xyz = tool_data[current_tool]['cut']

                if mode == 0:
                    rapid_xyz[0].extend([x, new_x, None])
                    rapid_xyz[1].extend([y, new_y, None])
                    rapid_xyz[2].extend([z, new_z, None])
                else:
                    cut_xyz[0].extend([x, new_x, None])
                    cut_xyz[1].extend([y, new_y, None])
                    cut_xyz[2].extend([z, new_z, None])
...
def visualize_gcode(file_path):
    try:
        tool_data, tool_order = parse_gcode_with_tools(file_path)
    ...
    # Палитра цветов для разных инструментов
    colors = [
        "blue", "red", "green", "orange", "magenta",
        "cyan", "brown", "purple", "olive", "gray"
    ]
    ...
    for idx, tool in enumerate(tool_order):
        ...
        # Рабочие ходы для этого инструмента
        if any(cut):
        ...
        # Холостые ходы для этого инструмента
        if any(rapid):
            fig.add_trace(go.Scatter3d(
    ...
    fig.update_layout(
        title=f'3D Визуализация: {file_name}',
        scene=dict(
...
def main():
    ...
    label = tk.Label(
        root,
        text="3D визуализация G-кода\nс разделением по инструментам",
        font=("Arial", 12),
        pady=20
    )
    label.pack()
